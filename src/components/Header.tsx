...
export default function Header() {
  ...
  const connectedCount = tableState?.players.filter((p) => p.connected).length ?? 0;

  return (
    <header className="flex items-center justify-between px-4 py-3 bg-charcoal/90 backdrop-blur-sm border-b border-charcoal-lighter">
      {/* Room Info */}
      <div className="flex items-center gap-3">
        <div className="text-gold font-bold text-lg sm:text-xl font-[Georgia] tracking-tight">
          DDM
        </div>
        <div className="flex items-center gap-2 bg-charcoal-lighter/50 rounded-lg px-2.5 py-1">
          <span className="text-cream/40 text-[10px] uppercase">Room</span>
          <span className="text-gold font-mono font-bold text-sm tracking-wider">{roomCode}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-1.5 h-1.5 rounded-full bg-casino-green" />
          <span className="text-cream/40 text-xs">{connectedCount} player{connectedCount !== 1 ? 's' : ''}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <AnimatePresence>
          {errorMessage && (
            <motion.div
              ...
              exit={{ opacity: 0, x: 20 }}
              className="text-xs text-casino-red bg-casino-red/10 px-2 py-1 rounded"
            >
              {errorMessage}
            </motion.div>
          )}
        ...
        <div className="flex flex-col items-end">
          <span className="text-cream/40 text-[10px] uppercase tracking-wider">Balance</span>
          <AnimatePresence mode="wait">
            <motion.span
              ...
              exit={{ y: 10, opacity: 0 }}
              className="text-gold font-bold text-lg sm:text-xl"
            >
      ...
      <div className="flex items-center gap-1 sm:gap-2">
        ...
        <div className="relative lg:hidden">
          ...
          {unreadChatCount > 0 && (
            <span className="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 min-w-[18px] flex items-center justify-center bg-casino-red text-cream text-[10px] font-bold rounded-full leading-none px-1">
        ...
        <button
          ...
          className={`px-2.5 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all duration-150
            ${isAway
...
function HeaderBtn({ onClick, title, children }: {
  ...
  return (
    <button
      onClick={onClick}
      title={title}
      className="p-2 rounded-lg text-cream/60 hover:text-gold hover:bg-charcoal-lighter transition-all duration-150"
    >
      {children}
    </button>
  );
